...
public class HttpAuthorizationTest {
  ...
  private void test(String header, HttpAuthorization... expect) {
    ...
    Assert.assertEquals(expect.length, result.size());
    IntStream.range(0, result.size()).forEach(i -> {
      Assert.assertEquals(expect[i].getType(), result.get(i).getType());
      Assert.assertEquals(expect[i].getCredential(), result.get(i).getCredential());
    });
  }
}
